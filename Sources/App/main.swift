...
let gameBuilder = SessionBuilder(spawner: Spawn.perChatID(updateType: [.message, .callbackQuery, .inlineQuery], chatType: [.group, .supergroup]),
														 idType: .chat,
														 session: GameSession.self,
														 setup: nil)

let playerBuilder = SessionBuilder(spawner: Spawn.perUserID(updateType: [.message, .inlineQuery, .callbackQuery]),
																	 idType: .user,
																	 session: PlayerSession.self,
																	 setup: nil)

playerBuilder.collision	= { _, _ in return .all }
pelican.addBuilder(gameBuilder)
pelican.addBuilder(playerBuilder)
